...
# read in the data --------------------------------------------------------
UMAP_ref_fetal <- read_tsv("out/table/azimuth_fetusref_CoordUMAP.tsv")
UMAP_ref2_fetal <- read_tsv("out/table/data.combined_NOT_annotated_fix_normalized_CoordUMAP_AzimuthHumanFetalRef_resolution_DoubletSinglet_harmonyMartina.tsv")
meta_ref_fetal <- read_tsv("out/table/azimuth_fetusref_Metadata.tsv")
meta_query_fetal <- read_tsv("out/table/data.combined_NOT_annotated_fix_normalized_meta_AzimuthHumanFetalRef_resolution_DoubletSinglet_harmonyMartina.tsv") %>%
...
UMAP_ref2_cortex <- read_tsv("out/table/data.combined_NOT_annotated_fix_normalized_CoordUMAP_AzimuthHumanCortexRef_resolution_DoubletSinglet_harmonyMartina.tsv")
meta_ref_cortex <- read_tsv("out/table/azimuth_humancortex_Metadata.tsv")
meta_query_cortex <- read_tsv("out/table/data.combined_NOT_annotated_fix_normalized_meta_AzimuthHumanCortexRef_resolution_DoubletSinglet_harmonyMartina.tsv") %>%
...
bind_rows(list(df_cortex,df_fetal)) %>% 
  ...
  theme_bw() + 
  geom_vline(xintercept = 0.75,col="red",linetype="dashed")
ggsave("out/image/ridges_fetal_vs_adult.pdf",width = 6,height = 5)
...
bind_rows(list(df_cortex,df_fetal)) %>% 
  ...
  geom_vline(xintercept = 0.75,col="red",linetype="dashed")
ggsave("out/image/ridges_fetal_vs_adult2.pdf",width = 6,height = 5)
...
df_summary %>% 
  write_tsv("out/table/df_summary_BS_fetal_vs_adult.tsv")
